...
class MapDBSessionsStore implements ISessionsStore {
    ...
    private IMap<String, PersistentSession> m_persistentSessions;
    //maps clientID->[MessageId -> guid]
    ...
    public void addNewSubscription(Subscription newSubscription) {
        ...
        m_db.getMap(clientSubscriptionKey(clientID)).set(newSubscription.getTopicFilter(), newSubscription);

        if (LOG.isTraceEnabled()) {
            LOG.trace("subscriptions_{}: {}", clientID, m_db.getMap(clientSubscriptionKey(clientID)));
        }
    }

    static String clientSubscriptionKey(String clientId)
    {
    ...
    public void removeSubscription(String topicFilter, String clientID) {
        ...
        if (m_db.getMap(clientSubscriptionKey(clientID)).isEmpty()) {
            return;
        }
        m_db.getMap(clientSubscriptionKey(clientID)).remove(topicFilter);
    }
    ...
    public void wipeSubscriptions(String clientID) {
        ...
        if (LOG.isTraceEnabled()) {
            LOG.trace("Subscription pre wipe: subscriptions_{}: {}", clientID, m_db.getMap(clientSubscriptionKey(clientID)));
        }
        m_db.getMap(clientSubscriptionKey(clientID)).clear();
        if (LOG.isTraceEnabled()) {
            LOG.trace("Subscription post wipe: subscriptions_{}: {}", clientID, m_db.getMap(clientSubscriptionKey(clientID)));
        }
    }
    ...
    public List<ClientTopicCouple> listAllSubscriptions() {
        ...
        for (String clientID : m_persistentSessions.keySet()) {
            ConcurrentMap<String, Subscription> clientSubscriptions = m_db.getMap(clientSubscriptionKey(clientID));
    ...
    public Subscription getSubscription(ClientTopicCouple couple) {
        ConcurrentMap<String, Subscription> clientSubscriptions = m_db.getMap(clientSubscriptionKey(couple.clientID));
    ...
    public boolean contains(String clientID) {
        return !m_db.getMap(clientSubscriptionKey(clientID)).isEmpty();
    }
    ...
    public void dropQueue(String clientID) {
        this.m_db.getQueue(clientID).destroy();
    }
    ...
    public MessageGUID mapToGuid(String clientID, int messageID) {
        ConcurrentMap<Integer, MessageGUID> messageIdToGuid = m_db.getMap(messageId2GuidsMapName(clientID));
